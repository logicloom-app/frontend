...
import { createAnnouncement } from "@/services/adminServices";
...
export default function CreateAnnouncementModal({ open, onClose }) {
  ...
  const createMutation = useMutation({
    ...
    onError: (error) => {
      toast({
        variant: "destructive",
        description: error?.response?.data?.error || "Failed to create announcement",
        className: "rounded-2xl",
      });
    },
  });
        ...
        <form onSubmit={formik.handleSubmit} className="flex flex-col gap-5">
          ...
          <DialogFooter>
            ...
            <Button
              ...
              className="rounded-2xl"
            >
              {createMutation.isPending ? <Spinner className="w-6 h-6" /> : "Create"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
